...
<template>
    <BcTable ref="BcTableRefComponent" class="my-table" height="300px" :tableConfig="tableConfig" @GetApiData="GetapiData">
        ...
        <el-table-column align="center" prop="performanceTrend" label="业绩走势">
            <template #default="scope">
                <el-input class="my-input" size="mini" v-if="scope.row.type === 'input'" v-model="scope.row.performanceTrend" placeholder="请输入业绩走势"></el-input>
                <span v-else>{{ scope.row.performanceTrend }}</span>
            </template>
        </el-table-column>
        <el-table-column align="center" prop="averageNav" label="单位净值">
            <template #default="scope">
                <el-input class="my-input" size="mini" v-if="scope.row.type === 'input'" v-model="scope.row.averageNav" placeholder="请输入单位净值"></el-input>
                <span v-else>{{ scope.row.averageNav }}</span>
            </template>
        </el-table-column>
        <el-table-column align="center" prop="accumulatedNet" label="累计净值">
            <template #default="scope">
                <el-input class="my-input" size="mini" v-if="scope.row.type === 'input'" v-model="scope.row.accumulatedNet" placeholder="请输入累计净值"></el-input>
                <span v-else>{{ scope.row.accumulatedNet }}</span>
            </template>
        </el-table-column>
...
<script setup>
    ...
    const addCheck = (scope) => {
        let { row } = scope;
        if (row.dataDate == "" || row.performanceTrend == "" || row.averageNav == "" || row.accumulatedNet == "") {
