<template>
    <BcTable ref="BcTableScanCode" :isFill="true" :tableConfig="tableConfigScanCode" @GetApiData="getActivitiesTable" @selection-change="selectionChange">
        <!--标头查询区-->
        <template v-slot:search>
            <BcTableQueryForm
                ...
                ref="checkform"
                :model="FormData"
                @queryBtn="() => $refs['BcTableScanCode'].startQueryBtn()"
                @resetBtn="() => $refs['BcTableScanCode'].resetQueryBtn()">
                <template #default>
        ...
        <template v-slot:action>
            ...
            <el-button-group style="float: right; margin: 0px 10px 0px 0px">
                <el-button size="mini" type="primary" @click="$router.push('/channelReward/detail-3?benefitType=3')">新增</el-button>
...
<script>
    ...
    export default {
        ...
        setup() {
            ...
            onMounted(() => {
                instance.refs["BcTableScanCode"].emitGetApiData();
            });
            const { tableConfigScanCode } = createdTableConfig();
