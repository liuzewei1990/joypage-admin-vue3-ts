...
export default function () {
    ...
    const form = reactive({
        ...
        /* **********************拼人数信息*************************** */
        // 拼团类型
        assembleType: routeForm.assembleType || "1",
        // 拼团人数
        assembleTotalNumbers: routeForm.assembleTotalNumbers || "",
        // 注水时间
        injectionWaterInterval: routeForm.injectionWaterInterval || "",

        /* **********************拼团信息*************************** */
        // 拼团金额
        targetAmount: routeForm.targetAmount || "",
        // 排序
        ...
        // 拼团时间段
        assembleMinutes: routeForm.assembleMinutes || "",
        ...
        ruleUrl: routeForm.ruleUrl ? routeForm.ruleUrl.split().map((item) => ({ url: item })) : [],
        // 详细规则
    ...
    const formRule = {
        ...
        targetAmount: [{ required: true, message: "该内容不能为空", trigger: "blur" }],
        assembleType: [{ required: true, message: "该内容不能为空", trigger: "change" }],
        assembleTotalNumbers: [{ required: true, message: "该内容不能为空", trigger: "blur" }],
        injectionWaterInterval: [{ required: true, message: "该内容不能为空", trigger: "blur" }],
        ...
        assembleMinutes: [{ required: true, message: "该内容不能为空", trigger: "blur" }],
        ...
        detailRule: [{ required: true, message: "该内容不能为空" }]
    };
    ...
    const postApi = async () => {
        ...
        if (pageType == "1") {
            let data = await _this.$apis.addActPerson(params);
            _this.$msg.success("添加成功");
        } else if (pageType == "2") {
            let data = await _this.$apis.updActPerson(params);
