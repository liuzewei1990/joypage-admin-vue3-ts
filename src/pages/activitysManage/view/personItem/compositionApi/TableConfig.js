...
export default function (BcTableRefComponent) {
    ...
    const updateRow = async (scope) => {
        ...
        let o = toRaw(row);
        let params = { id: o.id };
        let data = await _this.$apis.getActPersonById(params);
        router.push({ path: "/personItem/add", query: { row: encodeURIComponent(JSON.stringify(data)), pageType: "2" } });
    };

    // 复制
    const copyRow = async (scope) => {
        const { row } = scope;
        let o = toRaw(row);
        let params = { id: o.id };
        let data = await _this.$apis.getActPersonById(params);
        delete data.id;
        router.push({ path: "/personItem/add", query: { row: encodeURIComponent(JSON.stringify(data)), pageType: "3" } });
    };
    ...
    const deleteRow = async (scope) => {
        ...
        let data = await _this.$apis.delActPerson(params);
